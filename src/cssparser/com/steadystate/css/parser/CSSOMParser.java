...
public class CSSOMParser {
   ...
   public CSSOMParser(Parser parser) {
      if (parser instanceof SACParser) {
         this.parser_ = (SACParser) parser;
         ParserConfiguration config = ParserConfiguration.getInstance();
         if (parser instanceof SACParserCSS1) {
            config.setVersion(CSSVersion.CSS1);
         } else if (parser instanceof SACParserCSS2) {
            config.setVersion(CSSVersion.CSS2);
         } else if (parser instanceof SACParserCSS21) {
            config.setVersion(CSSVersion.CSS21);
         } else if (parser instanceof SACParserCSS3) {
            config.setVersion(CSSVersion.CSS3);
         }
      } else {
         this.parser_ = newParser();
      }
   }

   public static SACParser newParser() {
      return newParser((short) -1);
   }
   ...
   public static SACParser newParser(short version) {
      ...
      SACParser _parser;
      switch (version) {
         case CSSVersion.CSS1:
            _parser = new SACParserCSS1();
            break;
         case CSSVersion.CSS2:
            _parser = new SACParserCSS2();
            break;
         case CSSVersion.CSS21:
            _parser = new SACParserCSS21();
            break;
         case CSSVersion.CSS3:
            _parser = new SACParserCSS3();
            break;
         default:
            _parser = new SACParserCSS21();
    ...
    */
   public CSSOMParser(short version) {
      ParserConfiguration config = ParserConfiguration.getInstance();
      if (version == -1) {
         if (config.getVersion() != -1) {
            version = config.getVersion();
      ...
      switch (version) {
         case CSSVersion.CSS1:
            parser_ = new SACParserCSS1();
            break;
         case CSSVersion.CSS2:
            parser_ = new SACParserCSS2();
            break;
         case CSSVersion.CSS21:
